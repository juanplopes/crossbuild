#!/bin/bash
docker build -t juanplopes/crossbuild:manifest-amd64 --platform amd64 --build-arg ARCH=amd64/ .
docker push juanplopes/crossbuild:manifest-amd64

docker build -t juanplopes/crossbuild:manifest-arm64v8 --platform arm64 --build-arg ARCH=arm64/ .
